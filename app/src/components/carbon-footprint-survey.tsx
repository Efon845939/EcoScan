...
export function CarbonFootprintSurvey({ onBack, region, language }: CarbonFootprintSurveyProps) {
  ...
  const handleSubmit = async () => {
    ...
    startTransition(async () => {
      ...
      try {
        ...
        kgForPoints = aiAnalysisResult?.estimatedFootprintKg ?? 0;
      } catch (e) {
        console.error("AI analysis failed, falling back to deterministic", e);
        kgForPoints = 0; // Will be replaced by deterministic below
      }

      // If AI failed or gave a zero value, use deterministic calculation.
      if (kgForPoints === 0) {
        let detKg = computeKgDeterministic(regionKey, worstTransport, worstDiet, worstDrink, energyEnum);
        detKg = enforceWorstFloor(detKg, regionKey, worstTransport, worstDiet, worstDrink, energyEnum);
        kgForPoints = detKg;
      }
  ...
  const handleFinalizeWithReceipt = () => {
    if (userProfileRef && userProfile) {
        const currentPoints = userProfile.totalPoints ?? 0;
        // Reverse the base points and add the full bonus
        const finalPoints = (currentPoints - results.basePoints) + (results.basePoints * 5);
