...
function AppContainer({ initialStep = 'scan' }: { initialStep?: Step}) {
  ...
  const renderContent = () => {
    ...
    switch (step) {
      case 'survey':
        return <CarbonFootprintSurvey onBack={() => router.push('/')} region={region} language={language} />;
      case 'scan':
        return (
          <Card className="text-center">
            ...
            <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <SurveyButton cooldownEndsAt={lastSurveyTimestamp?.toDate().getTime() ? lastSurveyTimestamp.toDate().getTime() + 24 * 60 * 60 * 1000 : undefined} onClick={() => router.push('/carbon-footprint')} />
            </CardContent>
