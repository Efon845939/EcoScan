...
import { computeKgDeterministic, getRegionKey, RegionKey, TRANSPORT_KG, DIET_KG, DRINK_KG } from '@/lib/carbon-calculator';
...
5.  **Recommendations Quality:** The 'recommendations' should be specific, actionable, and long-term suggestions. They must be detailed and explain the "why" behind the suggestion. Give longer, more descriptive recommendations.
...
const analyzeFootprintFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    try {
      ...
      const regionKey = getRegionKey(input.region) as RegionKey;
